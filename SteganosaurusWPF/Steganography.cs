    ...
    class Steganography
    {
        ...
        public static BitmapSource InsertionWithAlgorithmStandard(string imagePath, string messagePath, string key)
        {
            ...
            for (int i = 0; i < messageExtended.Length; i++)
            {
                for (byte j = 0; j < 8; j++)
                {
                    pixels[i * 8 + j] = pixels[i * 8 + j].SetBit(0, messageExtended[i].GetBit(j));
                }
            }
        ...
        public static FileTemp ExtractionWithAlgorithmStandard(string imagePath, string key)
        {
            ...
            {
                for (int j=0;j<8;j++){
                    fileNameLengthBytes[i] = fileNameLengthBytes[i].SetBit(j, pixels[idx].GetBit(0));
                    idx++;
                }
            }
            ...
            for (int i = 0; i <filenameLength*2; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    fileNameBytes[i] = fileNameBytes[i].SetBit(j, pixels[idx].GetBit(0));
                    idx++;
                }
            }
            string fileName = "";
            ...
            {
                for (int j=0;j<8;j++){
                     messageLengthBytes[i] = messageLengthBytes[i].SetBit(j, pixels[idx].GetBit(0));
                    idx++;
                }
            }
            ...
            {
                for (int j=0;j<8;j++){
                    messageBytes[i] = messageBytes[i].SetBit(j, pixels[idx].GetBit(0));
                    idx++;
                }
            }
